...
function start(port) {
    ...
    if (cluster.isMaster && monster.config.server.cluster) {
        ...
        cluster.on('exit', function (worker, code) {
            console.log('worker ' + worker.process.pid + ' died at:', monster.base.date('yyyy-MM-dd HH:mm:ss'));
            if (code !== 2) {
                console.log('worker ' + worker.process.pid + ' restart at:', monster.base.date('yyyy-MM-dd HH:mm:ss'));
                cluster.fork();
            }
        });
    } else {
        http.createServer(router.route).listen(port || 80);
...
// 启动服务
start(monster.config.server.onPort);
